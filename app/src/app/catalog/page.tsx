  ...
  return (
    <main id="main-content" className="w-full px-4 py-8 sm:px-6 lg:px-8">
      <section className="zen-soft-in mx-auto max-w-md md:hidden">
        ...
        {groupedMovies.length ? <div className="mt-7 space-y-9">{groupedMovies.map((group) => <section key={group.key} aria-labelledby={`mobile-date-${group.key}`} className="zen-catalog-group"><h2 id={`mobile-date-${group.key}`} className="sticky top-0 z-10 -mx-1 border-b border-border bg-background/92 px-1 pb-2 pt-2 text-base font-semibold tracking-tight backdrop-blur-md">{group.label}</h2><div className="mt-5 grid grid-cols-2 gap-x-3 gap-y-7">{group.movies.map((movie) => <MobileMovieTile key={movie.id} id={movie.tmdb_id ?? 0} image={movie.poster_url} title={movie.title} author={movie.director || "Inconnu"} year={movie.release_year} rating={movie.rating} onDelete={deletingId === movie.id ? undefined : () => void handleDelete(movie.id)} />)}</div></section>)}</div> : <div className="py-16 text-center"><h2 className="font-semibold">{movies.length ? "Aucun film trouvé" : "Catalogue vide"}</h2><p className="mt-2 text-sm text-muted-foreground">{movies.length ? "Essaie une autre recherche." : "Recherche un film pour commencer."}</p></div>}
      </section>

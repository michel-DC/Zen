...
import { ArrowLeft, Star, User, Clapperboard } from "lucide-react";
              ...
              <>
                <div className="flex flex-col gap-2">
                  <h1 className="text-4xl md:text-5xl font-bold tracking-tight text-foreground">
                    {movie?.title}
                    {movie?.release_year && (
                      <span className="ml-3 font-light text-muted-foreground">({movie.release_year})</span>
                    )}
                  </h1>
                  ...
                  <div className="flex items-center gap-4 mt-2">
                    ...
                    <div className="flex items-center text-muted-foreground text-sm border-l border-border pl-4">
                       Par <span className="font-medium text-foreground ml-1">{movie?.director}</span>
                    </div>
                  </div>

                  <div className="flex flex-wrap gap-2 mt-4">
                    {movie?.genres.map(genre => (
                      <Badge key={genre} variant="secondary" className="px-3 py-1 font-normal opacity-80">
                        {genre}
                      </Badge>
                    ))}
                  </div>
                      ...
                      <div key={i} className="flex flex-col gap-3 group">
                        ...
                        <div className="flex flex-col">
                           ...
                           <div className="flex items-center justify-between mt-1">
                             <span className="text-[11px] font-mono text-muted-foreground uppercase">{col.hex}</span>
                             <span className="text-[11px] bg-muted px-2 py-0.5 rounded-full font-medium">{col.percentage}%</span>
                           </div>
                        </div>
                      </div>
                    ))}

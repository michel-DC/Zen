...
export default function SettingsPage() {
  ...
  return (
    <main id="main-content" className="zen-mobile-page mx-auto w-full max-w-xl px-5 py-8 pt-[max(2rem,env(safe-area-inset-top))] md:max-w-xl md:py-12">
      <p className="zen-mobile-kicker md:hidden">Zen</p>
      <h1 className="zen-mobile-title mt-2 md:mt-0 md:text-[2rem]">Réglages</h1>
      <section className="mt-9">
        <h2 className="text-sm font-semibold text-muted-foreground">Apparence</h2>
        <div className="mt-3 rounded-2xl bg-muted p-1.5">
          {themes.map(({ value, label, icon: Icon }) => (
            <button key={value} onClick={() => setTheme(value)} aria-pressed={mounted && theme === value} className="flex min-h-14 w-full items-center gap-3 rounded-xl px-4 text-left active:bg-background/70">
      ...
      <section className="mt-9">
        ...
        <div className="mt-3 overflow-hidden rounded-2xl bg-muted p-1.5">
          <button onClick={() => void install()} className="flex min-h-16 w-full items-center gap-3 rounded-xl px-4 text-left active:bg-background/70"><Download className="size-5 text-primary" /><span className="flex-1"><strong className="block font-medium">Installer Zen</strong><small className="mt-0.5 block text-sm text-muted-foreground">{installReady ? "Ajouter à l’écran d’accueil" : "Voir les instructions"}</small></span></button>
          <div className="mx-4 h-px bg-border" />
          <button onClick={() => void clear()} disabled={clearing} className="flex min-h-16 w-full items-center gap-3 rounded-xl px-4 text-left text-destructive active:bg-background/70 disabled:opacity-50"><Trash2 className="size-5" /><span className="flex-1"><strong className="block font-medium">Vider la mémoire locale</strong><small className="mt-0.5 block text-sm text-muted-foreground">Conserve le catalogue en ligne et le thème</small></span></button>
        </div>
      </section>
    </main>
  );

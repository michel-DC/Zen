...
function MovieChoice({ movie, onAdd, priority = false }: { movie: CatalogMovie; onAdd: () => void; priority?: boolean }) {
  return <article className="flex min-w-0 flex-col gap-3">
    <div className="relative aspect-2/3 overflow-hidden rounded-xl bg-muted"><MoviePoster src={movie.poster_url} alt={movie.title} sizes="(max-width: 640px) 45vw, 25vw" priority={priority} /></div>
    <div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="font-medium leading-tight">{movie.title}</h3><p className="mt-1 text-sm text-muted-foreground">Par {movie.director || "Inconnu"}</p></div><Button type="button" variant="secondary" size="icon" className="size-11 shrink-0 rounded-full" aria-label={`Ajouter ${movie.title} à la base`} title="Ajouter à la base" onClick={onAdd}><Plus /></Button></div>
  </article>;
}
...
export default function RecommendationsPage() {
  ...
  return <main id="main-content" className="zen-discover zen-mobile-page w-full px-5 pb-8 pt-[max(2rem,env(safe-area-inset-top))] sm:px-6 md:py-8 lg:px-8"><section className="space-y-6">
    ...
    <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_22rem]">
      ...
      <aside className="h-fit rounded-2xl bg-muted p-4 md:p-5 lg:sticky lg:top-24"><div className="flex items-baseline justify-between border-b border-border pb-3"><div><h2 className="text-lg font-semibold tracking-tight">Ma base</h2><p className="mt-1 text-xs text-muted-foreground">Les films qui donnent la direction.</p></div><span className="text-sm font-semibold text-primary">{sources.length}</span></div>{sources.length ? <ul className="mt-1">{sources.map((movie) => <li key={movie.id} className="flex min-h-14 items-center justify-between gap-3 border-b border-border py-2 last:border-0"><div className="min-w-0"><p className="truncate text-sm font-medium">{movie.title}</p><p className="truncate text-xs text-muted-foreground">{movie.director || "Inconnu"}</p></div><Button type="button" variant="ghost" size="icon" className="size-11" aria-label={`Retirer ${movie.title}`} onClick={() => removeSource(movie.id)}><X /></Button></li>)}</ul> : <p className="py-5 text-sm text-muted-foreground">Ajoute un film pour commencer.</p>}<details className="border-t border-border pt-4 text-sm"><summary className="flex min-h-11 cursor-pointer items-center">Affiner les résultats</summary><div className="mt-3 space-y-3"><Field orientation="horizontal"><FieldLabel htmlFor="include-animation">Inclure les animations</FieldLabel><Switch id="include-animation" checked={includeAnimation} onCheckedChange={setIncludeAnimation} /></Field><Field orientation="horizontal"><FieldLabel htmlFor="include-documentary">Inclure les documentaires</FieldLabel><Switch id="include-documentary" checked={includeDocumentary} onCheckedChange={setIncludeDocumentary} /></Field></div></details><Button className="mt-5 h-12 w-full rounded-full" disabled={!sources.length || searching} onClick={() => run()}><Sparkles data-icon="inline-start" />{searching ? "Recherche en cours…" : "Trouver des films similaires"}</Button></aside>
    </div>

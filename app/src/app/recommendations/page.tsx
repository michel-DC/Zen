...
export default function RecommendationsPage() {
  ...
  return <main id="main-content" className="w-full px-10 py-8"><section className="space-y-10">
    <PageHeading title="Recommandations" description="Pars de tes films préférés pour découvrir ceux qui te ressemblent." />
    <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_22rem]">
      <section className="space-y-5"><div className="space-y-3"><FieldLabel htmlFor="reference-search">Rechercher un film de référence</FieldLabel><div className="relative w-full"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input id="reference-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Titre ou réalisateur" className="pl-9" autoComplete="off" /></div></div>

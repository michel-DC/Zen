...
export default function AfterFilmDialog({ open, title, onOpenChange, onSave }: { open: boolean; title: string; onOpenChange: (open: boolean) => void; onSave: (payload: AfterFilmPayload) => Promise<void> }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[92dvh] overflow-y-auto rounded-[1.75rem] border-0 bg-background p-5 sm:max-w-lg"><DialogHeader><DialogTitle>Après {title}</DialogTitle><DialogDescription>Garde une trace légère de ce que tu viens de voir. Tu pourras compléter plus tard.</DialogDescription></DialogHeader><div className="mt-2 space-y-6"><div className="flex items-end justify-between gap-4"><div><label htmlFor="watched-at" className="text-sm font-medium">Visionné le</label><input id="watched-at" type="date" value={watchedAt} max={localToday()} onChange={(event) => setWatchedAt(event.target.value)} className="mt-2 block h-11 rounded-xl bg-muted px-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring" /></div><button type="button" onClick={() => setFavorite((value) => !value)} aria-pressed={favorite} className={`flex size-11 items-center justify-center rounded-full ${favorite ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`} aria-label={favorite ? "Retirer des favoris" : "Ajouter aux favoris"}><Heart className={favorite ? "size-5 fill-current" : "size-5"} /></button></div><div><p className="text-sm font-medium">Ta note</p><div className="mt-2"><RatingPicker value={rating} onChange={setRating} /></div></div><div><label htmlFor="impression" className="text-sm font-medium">Ton impression</label><textarea id="impression" value={impression} onChange={(event) => setImpression(event.target.value)} placeholder="Ce qui t’est resté…" className="mt-2 min-h-24 w-full resize-y rounded-[1.1rem] bg-muted p-4 text-base outline-none placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-ring" /></div><div><p className="text-sm font-medium">Ce que tu as ressenti</p><div className="mt-2 flex flex-wrap gap-2">{emotions.map(([value, label]) => <button key={value} type="button" aria-pressed={selectedEmotions.includes(value)} onClick={() => setSelectedEmotions((current) => toggle(current, value))} className={`min-h-10 rounded-full px-3 text-sm font-medium ${selectedEmotions.includes(value) ? "bg-primary text-primary-foreground" : "bg-muted text-foreground"}`}>{label}</button>)}</div></div><div><p className="text-sm font-medium">Ce que tu as aimé</p><div className="mt-2 flex flex-wrap gap-2">{aspects.map(([value, label]) => <button key={value} type="button" aria-pressed={selectedAspects.includes(value)} onClick={() => setSelectedAspects((current) => toggle(current, value))} className={`min-h-10 rounded-full px-3 text-sm font-medium ${selectedAspects.includes(value) ? "bg-primary text-primary-foreground" : "bg-muted text-foreground"}`}>{label}</button>)}</div></div><div className="grid grid-cols-2 gap-3 pt-1"><button type="button" disabled={pending} onClick={() => void save(true)} className="min-h-12 rounded-full bg-muted px-4 text-sm font-semibold disabled:opacity-50">Plus tard</button><button type="button" disabled={pending} onClick={() => void save(false)} className="min-h-12 rounded-full bg-primary px-4 text-sm font-semibold text-primary-foreground disabled:opacity-50">{pending ? "Enregistrement…" : "Enregistrer"}</button></div></div></DialogContent></Dialog>;
}
